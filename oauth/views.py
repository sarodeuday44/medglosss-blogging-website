...
from Medgloss.utils import send_email, get_md5, save_user_avatar
...
from Medgloss.blog_signals import oauth_user_login_signal
...
def authorize(request):
    ...
    if user:
        if not user.nikename or not user.nikename.strip():
            import datetime
            user.nikename = "Medgloss" + datetime.datetime.now().strftime('%y%m%d%I%M%S')
        try:
                ...
                if not author:
                    ...
                    if result[1]:
                        try:
                            ...
                            author.username = "Medgloss" + datetime.datetime.now().strftime('%y%m%d%I%M%S')
...
def emailconfirm(request, id, sign):
    ...
    with transaction.atomic():
        if oauthuser.author:
            ...
            if result[1]:
                ...
                ) else "Medgloss" + datetime.datetime.now().strftime('%y%m%d%I%M%S')
                author.save()
    ...
    site = get_current_site().domain
    content = '''
     <p>Congratulations, you've successfully bound your mailbox, and you can use {type} to log on to this site without password. Welcome to this site </p>

                <a href="{url}" rel="bookmark">{url}</a>

                Thank you again!
                <br />
                If the above link doesn't be open, copy this link to the browser.
                {url}
    '''.format(type=oauthuser.type, url='http://' + site)

    send_email(emailto=[oauthuser.email, ],
               title='Congratulations on your binding success!', content=content)
...
class RequireEmailView(FormView):
    ...
    def form_valid(self, form):
        ...
        content = """
                <p>Please bind your mailbox by clicking the link below</p>

                <a href="{url}" rel="bookmark">{url}</a>

                Thank you again!
                <br />
                If the above link doesn't open, copy this link to the browser.
                {url}
                """.format(url=url)
        send_email(emailto=[email, ], title='send email', content=content)
        url = reverse('oauth:bindsuccess', kwargs={
            'oauthid': oauthid
        })
        url = url + '?type=email'
